...
export async function createSchema(c: duckdb.AsyncDuckDBConnection): Promise<void> {
  log("Resetting schema (scratch session).");

  for (const t of ["audit_log_records", "audit_sql_templates_stripped"])
    await c.query(`DROP TABLE IF EXISTS ${t}`);

  await c.query(
    "CREATE TABLE IF NOT EXISTS audit_log_records (dataset_id VARCHAR NOT NULL, record_id BIGINT NOT NULL, event_time_ms BIGINT, is_internal BOOLEAN, query_id VARCHAR, user_name VARCHAR, client_ip VARCHAR, fe_ip VARCHAR, db_name VARCHAR, state VARCHAR, error_code INTEGER, error_message VARCHAR, query_time_ms BIGINT, cpu_time_ms BIGINT, scan_bytes BIGINT, scan_rows BIGINT, return_rows BIGINT, peak_memory_bytes BIGINT, workload_group VARCHAR, cloud_cluster_name VARCHAR, stmt_raw VARCHAR, stripped_template_id INTEGER);"
  );
  await c.query(
